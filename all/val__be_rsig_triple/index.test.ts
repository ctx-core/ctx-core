...
import { val__be_rsig_triple_ } from './index.js'
test('val__be_rsig_triple_|+base_name|+rsig__new|+be__params', ()=>{
	const ctx = ctx__new()
...
test('val__be_rsig_triple_|-base_name|+rsig__new|-be__params', ()=>{
	const [
		...
		foobar_,
		foobar__set,
	] = val__be_rsig_triple_(undefined, ()=>1)
	const ctx = ctx__new()
	equal(foobar$_(ctx)._, 1)
	equal(foobar_(ctx), 1)
	foobar__set(ctx, 2)
